...
def read_and_extract_features(reader, count, period, features,message=''):
...
def main():
    ...
    (train_X, train_y, train_names, train_ts) = read_and_extract_features(
        train_reader, n_train, args.period, args.features,message='Getting Train')

    (val_X, val_y, val_names, val_ts) = read_and_extract_features(
        val_reader, n_val, args.period, args.features,message='Getting Val')

    (test_X, test_y, test_names, test_ts) = read_and_extract_features(
        test_reader, test_reader.get_number_of_examples(), args.period, args.features,message='Getting Test')
    ...
    train_ts1 = train_ts[:split]
    train_X2 = train_X[split:]
    train_y2 = train_y[split:]
    train_ts2 = train_ts[split:]

    np.save('train_X1.npy',train_X1)
    np.save('train_y1.npy',train_y1)
    np.save('train_ts1.npy',train_ts1)
    np.save('train_X2.npy',train_X2)
    np.save('train_y2.npy',train_y2)
    np.save('train_ts2.npy',train_ts2)

    np.save('val_X.npy',val_X)
    np.save('val_y.npy',val_y)
    np.save('val_ts.npy',val_ts)
